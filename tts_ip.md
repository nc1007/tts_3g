...
Mặc định hệ thống khởi chạy trên:
...
```bash
curl -X POST 'http://localhost:7009/create_ID' \
...
```bash
curl -X POST 'http://localhost:7009/synthesize_ID_v2' \
  -H 'Content-Type: multipart/form-data' \
...
```bash
curl -X GET 'http://localhost:7009/list_ids' \
  -H 'accept: application/json'
```
...
```bash
curl -X DELETE 'http://localhost:7009/delete_id/20260520_131005_4821' \
  -H 'accept: application/json'
```
